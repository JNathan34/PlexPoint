...
function plexFetchTimeoutMs(env: PlexEnv): number {
  const configured = Number(env.PLEX_FETCH_TIMEOUT_MS);
  if (!Number.isFinite(configured) || configured <= 0) return DEFAULT_PLEX_FETCH_TIMEOUT_MS;
...
async function getLibrarySectionItemCount(env: PlexEnv, type: PlexMediaType, sectionId: string): Promise<number> {
  ...
  return cached(key, 60_000, async () => {
    ...
    const mediaContainer = ((parsed as any).MediaContainer ?? parsed) as Record<string, unknown>;
    const total =
      pickFirstNumber(mediaContainer, ["@_totalSize", "totalSize", "@_size", "size"]) ??
...
async function getLibraryItemCount(env: PlexEnv, type: PlexMediaType): Promise<number> {
...
export async function getPlexLibraryCounts(env: PlexEnv): Promise<PlexLibraryCounts> {
  const safeCount = async (type: PlexMediaType) => {
    try {
